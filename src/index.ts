...
function formataTelefone(telefone: string) {
  ...
  return telNumeros.substring(0, 2) === '55' ? telNumeros.substring(2) : telNumeros;
}
...
function formataDataPtBR(
  ...
  if (typeof data === 'string') {
    data = new Date(data);
    if (isNaN(+data))
      throw new Error('Data inválida. Precisa ser enviada no formato YYYY-MM-DD');
  }
...
function removeExtensaoArquivo(nomeArquivo: string) {
  if (typeof nomeArquivo !== 'string') throw new Error('O parametro precisa ser string');
